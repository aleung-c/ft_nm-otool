...
void	fill_outputs_64(t_nm *nm, int nsyms, int symoff, int stroff)
{
	...
	list = (void *)nm->file_ptr + symoff;
	string_table = (void *)nm->file_ptr + stroff;
	i = 0;
	nm->output_list = NULL;
	while (i < nsyms)
	{
		...
		{
			new_output->sym_type = get_symbol_section_type_64(list[i].n_sect, nm->file_ptr); // need to go check the section.
		}
		else
			new_output->sym_type = 'S'; // in an uninitialized data section.
		// print symbol name;
		new_output->sym_str = string_table + list[i].n_un.n_strx;
...
void	handle_64(t_nm *nm, char *file_ptr)
{
	...
	while (i < ncmds)
	{
		...
		if (lc->cmd == LC_SYMTAB)
		{
			...
			fill_outputs_64(nm, symtab->nsyms, symtab->symoff, symtab->stroff);
		}
		lc = (void *)lc + lc->cmdsize;
		i++;
	}
